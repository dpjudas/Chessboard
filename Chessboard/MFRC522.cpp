...
MFRC522::MFRC522(SPIDevice* spi) : spi(spi)
{
}
...
{
	char data[2], result[2];
	data[0] = 0x80 | ((reg) & 0x7E);
	spi->transfer(data, result, 2);
	return result[1];
}
	...
	uint8_t rxAlign)	///< Only bit positions rxAlign..7 in values[0] are updated.
{
	...
	address = 0;
	spi->transfer(&address, &value, 1); // Read the final byte. Send 0 to stop reading.
	values[index] = value;
}
...
{
	/*if (bcm2835_gpio_lev(RSTPIN) == LOW)
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
uint8_t MFRC522::MIFARE_Ultralight_Write(
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
uint8_t MFRC522::MIFARE_TwoStepHelper(
 ...
 *
 */
const std::string MFRC522::GetStatusCodeName(
	uint8_t code)	///< One of the StatusCode enums.
{
	switch (code)
	{
 ...
 *
 */
const std::string MFRC522::PICC_GetTypeName(
